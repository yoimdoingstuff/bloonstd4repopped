...
TEST_CASE(SwfParserDecodesLossless32BitToRgba) {
    ...
    // DefineBitsLossless2: format 5, a single semi-transparent red pixel in ARGB.
    const std::vector<uint8_t> rawPixels = {128, 255, 0, 0};
    std::vector<uint8_t> payload = {20, 0, 5, 1, 0, 1, 0};
    const std::vector<uint8_t> compressed = zlibStored(rawPixels);
    payload.insert(payload.end(), compressed.begin(), compressed.end());
    appendTag(swf, 36, payload);
    appendTag(swf, 0, {});
    patchFileLength(swf);
    ...
    TEST_ASSERT_EQ(parser.images()[0].width, static_cast<uint32_t>(1));
    TEST_ASSERT_EQ(parser.images()[0].height, static_cast<uint32_t>(1));
    TEST_ASSERT_EQ(parser.images()[0].data.size(), static_cast<size_t>(4));
    TEST_ASSERT_EQ(parser.images()[0].data[0], static_cast<uint8_t>(255));
    TEST_ASSERT_EQ(parser.images()[0].data[1], static_cast<uint8_t>(0));
    TEST_ASSERT_EQ(parser.images()[0].data[2], static_cast<uint8_t>(0));
    TEST_ASSERT_EQ(parser.images()[0].data[3], static_cast<uint8_t>(128));
}

TEST_CASE(SwfParserDecodesLossless16BitRgb15) {
    std::vector<uint8_t> swf = {'F','W','S',10,0,0,0,0,0x28,0x05,0x00,0x50,0x00,0x3C,0x01,0x00};

    // DefineBitsLossless: format 4, one 555 pixel: pure green.
    const uint16_t rgb15 = static_cast<uint16_t>(31u << 5);
    const std::vector<uint8_t> rawPixels = {static_cast<uint8_t>(rgb15 & 0xFF), static_cast<uint8_t>(rgb15 >> 8), 0, 0};
    std::vector<uint8_t> payload = {21, 0, 4, 1, 0, 1, 0};
    const std::vector<uint8_t> compressed = zlibStored(rawPixels);
    payload.insert(payload.end(), compressed.begin(), compressed.end());
    appendTag(swf, 20, payload);
    appendTag(swf, 0, {});
    patchFileLength(swf);
    ...
    TEST_ASSERT(parser.parse(swf.data(), swf.size(), error));
    TEST_ASSERT_EQ(parser.images().size(), static_cast<size_t>(1));
    TEST_ASSERT(parser.images()[0].data[1] > 240);
    TEST_ASSERT_EQ(parser.images()[0].data[0], static_cast<uint8_t>(0));
    TEST_ASSERT_EQ(parser.images()[0].data[2], static_cast<uint8_t>(0));
    TEST_ASSERT_EQ(parser.images()[0].data[3], static_cast<uint8_t>(255));
}
