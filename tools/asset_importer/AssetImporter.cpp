...
// These aliases are intentionally limited to towers that actually belong to
// BTD4 or its documented expansion/mobile content. Do not silently map later
// BTD5/BTD6-only towers just because a similarly named source symbol exists.
...
void inspectIpa(const std::string& path,ImportReport& report,const std::function<void(const std::string&)>& emitLog){
    ...
    try {
        emitLog("[Importer] Opening IPA archive for read-only indexing...");
        ZipArchive archive; std::string archiveError;
        ...
        if(!archive.contains("Info.plist")){
            for(const auto& entry:archive.entries()){
                ...
                if(n.rfind("payload/",0)==0&&n.size()>=9&&n.rfind("info.plist")==n.size()-9){
                    ...
                    if(archive.readEntry(entry.name,data,archiveError)){addFeature(report,"Readable app Info.plist");emitLog("[Importer] Read app Info.plist: "+entry.name+" ("+std::to_string(data.size())+" bytes).");}
                    else report.warnings.push_back("Could not read app Info.plist: "+archiveError);
                    break;
                }
            }
        }
    } catch(const std::bad_alloc&) {
        report.warnings.push_back("IPA indexing ran out of memory; continuing without indexing the archive contents.");
        emitLog("[Importer Warning] IPA indexing ran out of memory; continuing with SWF-only import.");
    } catch(const std::exception& e) {
        report.warnings.push_back(std::string("IPA indexing failed: ")+e.what());
        emitLog(std::string("[Importer Warning] IPA indexing failed: ")+e.what());
    } catch(...) {
        report.warnings.push_back("IPA indexing failed with an unknown error.");
        emitLog("[Importer Warning] IPA indexing failed with an unknown error.");
    }
}
