...
void writeBmpHeader(std::ofstream& out, uint32_t width, uint32_t height) {
    ...
    int32_t h = -static_cast<int32_t>(height); // Top-down
    uint16_t planes = 1;
    uint16_t bpp = 32;
    uint32_t compression = 0; // BI_RGB
    uint32_t xPels = 2835; // 72 DPI
    uint32_t yPels = 2835;
    uint32_t clrUsed = 0;
    ...
    out.write(reinterpret_cast<const char*>(&xPels), 4);
    out.write(reinterpret_cast<const char*>(&yPels), 4);
...
bool AssetConverter::saveImage(const swf::SwfImage& image, const std::string& outputPath) {
    ...
    if (image.format == swf::ImageFormat::JPEG) {
        out.write(reinterpret_cast<const char*>(image.data.data()), image.data.size());
        return true;
    } else {
        ...
        if (image.data.size() >= w * h * 4) {
            // Flash ARGB to BMP BGRA order
            std::vector<uint8_t> bgra(w * h * 4);
            for (size_t i = 0; i < w * h; ++i) {
                uint8_t a = image.data[i * 4 + 0];
                uint8_t r = image.data[i * 4 + 1];
                uint8_t g = image.data[i * 4 + 2];
                uint8_t b = image.data[i * 4 + 3];
                bgra[i * 4 + 0] = b;
                bgra[i * 4 + 1] = g;
                bgra[i * 4 + 2] = r;
                bgra[i * 4 + 3] = a;
...
bool AssetConverter::saveSound(const swf::SwfSound& sound, const std::string& outputPath) {
    ...
    if (sound.format == swf::SoundFormat::MP3) {
        // Skip first 2 bytes (SoundEnvelopeSeek in SWF MP3 streams)
        size_t offset = (sound.data.size() > 2) ? 2 : 0;
        out.write(reinterpret_cast<const char*>(sound.data.data() + offset), sound.data.size() - offset);
        return true;
    } else {
        // Standard PCM WAV
        uint16_t numChannels = sound.isStereo ? 2 : 1;
        uint16_t bitsPerSample = sound.is16Bit ? 16 : 8;
        writeWavHeader(out, sound.sampleRate, numChannels, bitsPerSample, static_cast<uint32_t>(sound.data.size()));
        out.write(reinterpret_cast<const char*>(sound.data.data()), sound.data.size());
        return true;
    }
}
...
std::string AssetConverter::normalizeIdentifier(const std::string& rawName, const std::string& fallbackPrefix, uint16_t id) {
    if (rawName.empty()) {
        return fallbackPrefix + "_" + std::to_string(id);
    }

    std::string result;
    result.reserve(rawName.size() + 4);

    for (size_t i = 0; i < rawName.size(); ++i) {
        char c = rawName[i];
        if (std::isupper(static_cast<unsigned char>(c))) {
            if (i > 0 && rawName[i - 1] != '_' && !std::isupper(static_cast<unsigned char>(rawName[i - 1]))) {
                result.push_back('_');
            }
