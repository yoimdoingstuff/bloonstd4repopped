...
fs::path locateExecutable(const fs::path& cmakeDir){
    std::error_code ec;
    const fs::path candidates[]={cmakeDir/"btd4_game.exe",cmakeDir/"Release"/"btd4_game.exe",cmakeDir/"Debug"/"btd4_game.exe"};
    for(const auto& candidate:candidates) if(fs::is_regular_file(candidate,ec)) return candidate;
    return {};
}
}
...
BuildResult WindowsPlatform::package(const std::string& gameEdition){BuildResult r;const fs::path root=sourceRoot();const fs::path cmakeDir=buildRoot()/"cmake";const fs::path executable=locateExecutable(cmakeDir);const fs::path packageDir=buildRoot()/"Playable";const fs::path dataRoot=root/"game_data"/"Windows";if(root.empty()||executable.empty()){r.message="Windows executable was not produced. Checked single-config and multi-config CMake output locations under "+cmakeDir.string();return r;}std::error_code ec;fs::remove_all(packageDir,ec);fs::create_directories(packageDir,ec);if(ec){r.message="Could not create Windows package directory: "+ec.message();return r;}fs::copy_file(executable,packageDir/executable.filename(),fs::copy_options::overwrite_existing,ec);if(ec){r.message="Could not copy Windows executable: "+ec.message();return r;}
    ...
    const fs::path possibleDlls[]={cmakeDir/"SDL2.dll",cmakeDir/"Release"/"SDL2.dll",cmakeDir/"Debug"/"SDL2.dll"};
