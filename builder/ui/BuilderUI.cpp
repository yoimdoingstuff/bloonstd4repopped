...
void BuilderUI::renderSourceFilesSection() {
    if (ImGui::CollapsingHeader("1. Source Game Files", ImGuiTreeNodeFlags_DefaultOpen)) {
        ...
        ImGui::TextDisabled("Drop .SWF / .IPA files here");
        ImGui::EndChild();
    }
}
...
void BuilderUI::renderFeaturesSection() {
    if (ImGui::CollapsingHeader("2. Detected & Enabled Features", ImGuiTreeNodeFlags_DefaultOpen)) {
        static bool featBase = true, featTowers = true, featBloons = true;
        static bool featMaps = true, featSounds = true, featAchievements = true;
        ImGui::Checkbox("Base Game", &featBase); ImGui::SameLine();
        ImGui::Checkbox("Towers", &featTowers); ImGui::SameLine(); ImGui::Checkbox("Bloons", &featBloons);
...
void BuilderUI::renderPlatformSection() {
    if (ImGui::CollapsingHeader("3. Target Platform", ImGuiTreeNodeFlags_DefaultOpen)) {
        ...
        for (const auto& backend : backends) {
            ...
            if (ImGui::RadioButton(backend->name().c_str(), isSelected)) m_project.config().targetPlatform = backend->name();
            ImGui::SameLine();
            ImGui::TextColored(available ? ImVec4(0.2f,1.0f,0.2f,1.0f) : ImVec4(1.0f,0.3f,0.3f,1.0f), available ? "[OK Toolchain detected]" : "[! Toolchain unavailable]");
...
void BuilderUI::renderActionButtons() {
    ...
    ImGui::SameLine(); if (ImGui::Button("Build Game", ImVec2(130, 32))) triggerBuild();
    ImGui::SameLine(); if (ImGui::Button("Save Project", ImVec2(110, 32))) {
...
void BuilderUI::triggerBuild() {
    ...
    if (!backend) { appendLog("[Build] Unknown target platform: " + m_project.config().targetPlatform); return; }
    appendLog("========================================="); appendLog("[Build] Initiating build for: " + backend->name());
    ...
    appendLog("[Build Success] Target " + backend->name() + " successfully built and packaged!"); appendLog("=========================================");
}
...
void BuilderUI::triggerImport() {
    ...
    } else appendLog("[Pipeline Error] Import failed: " + report.errorMessage);
    appendLog("=========================================");
}

void BuilderUI::renderLogsSection() {
    ImGui::Text("Build & Toolchain Logs"); ImGui::SameLine(ImGui::GetColumnWidth() - 90);
    ...
    if (m_autoScrollLogs && ImGui::GetScrollY() >= ImGui::GetScrollMaxY()) ImGui::SetScrollHereY(1.0f);
    ImGui::EndChild();
}
