...
bool AssetManager::initialize(const IFileSystem& fsBridge, const std::string& dataDirectory) {
    ...
    if (fs::is_directory(root, ec)) {
        ...
        for (const auto& entry : fs::directory_iterator(root, fs::directory_options::skip_permission_denied, ec)) {
            ...
            if (fs::is_regular_file(manifest, ec)) manifests.push_back(manifest);
        }
        std::sort(manifests.begin(), manifests.end());
        if (manifests.size() == 1) {
            const fs::path manifest = manifests.front();
            const std::string manifestPath = manifest.generic_string();
            if (m_manifest.loadFromFile(fsBridge, manifestPath, err)) {
                m_dataDir = manifest.parent_path().generic_string();
                m_hasManifest = true;
                return true;
            }
        }
...
PlaceholderColor AssetManager::getPlaceholderColor(const std::string& assetId) const {
    auto it=m_fallbackColors.find(assetId); return it!=m_fallbackColors.end()?it->second:PlaceholderColor{200,200,200};
}

size_t AssetManager::preloadTextures(IRenderer& renderer) {
    if (!m_hasManifest) return 0;
    size_t loaded=0;
    for(const auto& pair:m_manifest.textures) if(renderer.loadTexture(pair.first,m_dataDir+"/"+pair.second)) ++loaded;
    return loaded;
}

std::string AssetManager::getBloonAssetId(BloonType type) {
    switch(type){case BloonType::Red:return "bloon_red";case BloonType::Blue:return "bloon_blue";case BloonType::Green:return "bloon_green";case BloonType::Yellow:return "bloon_yellow";case BloonType::Pink:return "bloon_pink";case BloonType::Black:return "bloon_black";case BloonType::White:return "bloon_white";case BloonType::Lead:return "bloon_lead";case BloonType::Rainbow:return "bloon_rainbow";case BloonType::Ceramic:return "bloon_ceramic";case BloonType::MOAB:return "bloon_moab";default:return "";}
}

std::string AssetManager::getTowerAssetId(TowerType type) {
    switch(type){case TowerType::DartMonkey:return "tower_dart_monkey";case TowerType::TackShooter:return "tower_tack_shooter";case TowerType::SniperMonkey:return "tower_sniper_monkey";case TowerType::BoomerangThrower:return "tower_boomerang";case TowerType::BombTower:return "tower_bomb_tower";case TowerType::SuperMonkey:return "tower_super_monkey";default:return "";}
}

std::string AssetManager::getProjectileAssetId(ProjectileType type) {
    switch(type){case ProjectileType::Dart:return "projectile_dart";case ProjectileType::Tack:return "projectile_tack";case ProjectileType::Bomb:return "projectile_bomb";case ProjectileType::Boomerang:return "projectile_boomerang";case ProjectileType::SniperShot:return "projectile_bullet";case ProjectileType::Laser:return "projectile_laser";case ProjectileType::Plasma:return "projectile_plasma";default:return "";}
}

void AssetManager::drawBloon(IRenderer& renderer,const Bloon& bloon) const {
    if(!bloon.active)return; std::string id=getBloonAssetId(bloon.type);
    if(renderer.hasTexture(id)) renderer.drawSprite(id,bloon.x-bloon.radius,bloon.y-bloon.radius,bloon.radius*2.0f,bloon.radius*2.0f);
    else { PlaceholderColor pc=getPlaceholderColor(id); Color c{pc.r,pc.g,pc.b,255}; renderer.drawCircle(bloon.x,bloon.y-1.0f,bloon.radius,c,true); renderer.drawCircle(bloon.x,bloon.y-1.0f,bloon.radius,Color::black(),false); renderer.drawCircle(bloon.x-bloon.radius*.35f,bloon.y-bloon.radius*.45f,1.5f,Color::white(),true); renderer.drawRect(bloon.x-1.0f,bloon.y+bloon.radius-1.0f,2.0f,2.0f,c,true); }
}

void AssetManager::drawTower(IRenderer& renderer,const Tower& tower,bool isSelected) const {
    if(isSelected) DebugRenderer::drawTowerRange(renderer,tower.x(),tower.y(),tower.range()); std::string id=getTowerAssetId(tower.type());
    if(renderer.hasTexture(id)) renderer.drawSprite(id,tower.x()-16.0f,tower.y()-16.0f,32.0f,32.0f);
    else { PlaceholderColor pc=getPlaceholderColor(id); Color c{pc.r,pc.g,pc.b,255}; renderer.drawCircle(tower.x(),tower.y(),12.0f,c,true); renderer.drawCircle(tower.x(),tower.y(),12.0f,Color::black(),false); renderer.drawCircle(tower.x(),tower.y(),4.0f,Color::white(),true); renderer.drawCircle(tower.x(),tower.y(),2.0f,Color::black(),true); }
}

void AssetManager::drawProjectile(IRenderer& renderer,const Projectile& proj) const {
    if(!proj.active)return; std::string id=getProjectileAssetId(proj.type);
    if(renderer.hasTexture(id)) renderer.drawSprite(id,proj.x-4.0f,proj.y-4.0f,8.0f,8.0f);
    else if(proj.type==ProjectileType::Bomb){renderer.drawCircle(proj.x,proj.y,4.0f,Color::black(),true);renderer.drawCircle(proj.x,proj.y,4.0f,Color::red(),false);}
    else if(proj.type==ProjectileType::Plasma) renderer.drawCircle(proj.x,proj.y,5.0f,Color::cyan(),true);
    else {float len=6.0f,speed=std::sqrt(proj.vx*proj.vx+proj.vy*proj.vy);float dx=speed>.001f?proj.vx/speed:1.0f,dy=speed>.001f?proj.vy/speed:0.0f;renderer.drawLine(proj.x,proj.y,proj.x-dx*len,proj.y-dy*len,Color::yellow());}
}
...
void AssetManager::drawHUD(IRenderer& renderer,const Economy& economy,int currentRound,size_t totalRounds,double fps,TowerType selectedPlacementType,bool hasPlacement) const {
    renderer.drawRect(0,0,400,22,{0,0,0,180},true); renderer.drawText("LIVES: "+std::to_string(economy.lives()),8,6,1,Color::red()); renderer.drawText("CASH: $"+std::to_string(economy.cash()),100,6,1,Color::yellow()); renderer.drawText("ROUND: "+std::to_string(currentRound)+"/"+std::to_string(totalRounds),210,6,1,Color::white()); renderer.drawText("FPS: "+std::to_string((int)std::round(fps)),340,6,1,Color::green());
    renderer.drawRect(400,0,80,272,{20,20,20,230},true); renderer.drawLine(400,0,400,272,{60,60,60,255}); renderer.drawText("TOWERS",412,6,1,Color::cyan());
    static const TowerType towers[]={TowerType::DartMonkey,TowerType::TackShooter,TowerType::BombTower,TowerType::BoomerangThrower,TowerType::SniperMonkey,TowerType::SuperMonkey}; float y=22;
    for(TowerType tt:towers){auto stats=getTowerBaseStats(tt);bool sel=hasPlacement&&selectedPlacementType==tt;bool afford=economy.canAfford(stats.cost);Color bg=sel?Color{60,100,160,255}:(afford?Color{45,45,45,255}:Color{30,30,30,255});renderer.drawRect(404,y,72,36,bg,true);renderer.drawRect(404,y,72,36,sel?Color::cyan():Color{80,80,80,255},false);std::string n;switch(tt){case TowerType::DartMonkey:n="DART";break;case TowerType::TackShooter:n="TACK";break;case TowerType::BombTower:n="BOMB";break;case TowerType::BoomerangThrower:n="RANG";break;case TowerType::SniperMonkey:n="SNIP";break;case TowerType::SuperMonkey:n="SPER";break;}renderer.drawText(n,408,y+4,1,sel?Color::white():(afford?Color::white():Color{120,120,120,255}));renderer.drawText("$"+std::to_string(stats.cost),408,y+18,1,afford?Color::yellow():Color::red());y+=36;}
}
