...
void AssetManager::drawHUD(IRenderer& renderer,const Economy& economy,int currentRound,size_t totalRounds,double fps,TowerType selectedPlacementType,bool hasPlacement) const{renderer.drawRect(0,0,400,22,{0,0,0,180},true);renderer.drawText("LIVES: "+std::to_string(economy.lives()),8,6,1,Color::red());renderer.drawText("CASH: $"+std::to_string(economy.cash()),100,6,1,Color::yellow());renderer.drawText("ROUND: "+std::to_string(currentRound)+"/"+std::to_string(totalRounds),210,6,1,Color::white());renderer.drawText("FPS: "+std::to_string((int)std::round(fps)),340,6,1,Color::green());renderer.drawRect(400,0,80,272,{20,20,20,230},true);renderer.drawLine(400,0,400,272,{60,60,60,255});renderer.drawRect(404,1,72,18,{35,45,55,255},true);renderer.drawRect(404,1,72,18,Color::cyan(),false);renderer.drawText("EDIT",431,6,1,Color::white());static const TowerType towers[]={TowerType::DartMonkey,TowerType::TackShooter,TowerType::BombTower,TowerType::BoomerangThrower,TowerType::SuperMonkey};float y=22;for(TowerType tt:towers){auto stats=getTowerBaseStats(tt);bool sel=hasPlacement&&selectedPlacementType==tt;bool afford=economy.canAfford(stats.cost);Color bg=sel?Color{60,100,160,255}:(afford?Color{45,45,45,255}:Color{30,30,30,255});renderer.drawRect(404,y,72,36,bg,true);renderer.drawRect(404,y,72,36,sel?Color::cyan():Color{80,80,80,255},false);const std::string logical=getTowerAssetId(tt);const std::string id=findImportedTextureId(m_manifest,renderer,logical);if(!id.empty())renderer.drawSprite(id,406,y+2,28,28);std::string n;switch(tt){case TowerType::DartMonkey:n="DART";break;case TowerType::TackShooter:n="TACK";break;case TowerType::BombTower:n="BOMB";break;case TowerType::BoomerangThrower:n="RANG";break;case TowerType::SuperMonkey:n="SPER";break;default:n="";break;}renderer.drawText(n,436,y+4,1,sel?Color::white():(afford?Color::white():Color{120,120,120,255}));renderer.drawText("$"+std::to_string(stats.cost),436,y+18,1,afford?Color::yellow():Color::red());y+=36;}}
}
